...
games <- purrr::map_dfr(2001:2020, 
                        function(x){
                          games <- cfbscrapR::cfb_game_info(x)
...
git2r::commit(repo, message = glue::glue("Updated schedules at {Sys.time()} using cfbscrapR")) # commit the staged files with the chosen message
